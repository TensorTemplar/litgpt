...
class LazySFTDataset(Dataset):
    """A generic lazy-loading dataset for supervised fine-tuning with `input_ids` and `labels`.

    This dataset reads data lazily from a data source without loading all data into memory.
    Each sample is read on-the-fly when ``__getitem__`` is called.

    Args:
        data_source: An object that implements ``__getitem__`` and ``__len__``, serving as the data source.
        tokenizer: The tokenizer to use. Should match the one that was used to pretrain the model.
        prompt_style: The style to apply to prompts. See `litgpt.prompts` for a list of available styles.
        max_seq_length: Truncate sequences that are longer than this value. By default, no truncation is applied.
        mask_prompt: Whether to mask the prompt section from the label (with ``ignore_index``).
    ...
    def __getitem__(self, idx: int) -> Dict[str, Tensor]:
        ...
        encoded_prompt_and_response = torch.cat((encoded_prompt, encoded_response)).long()
